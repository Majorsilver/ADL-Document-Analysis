...
class YOLOClassifier(nn.Module):
    def __init__(self):
        ...
        # Comment out since it is fine tunning
        for p in self.features.parameters(): p.requires_grad = False # Freeze backbone
        ...
        self.head = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.4),
            nn.Linear(128, 1) # <--- CHANGE THIS FROM CONFIG["num_classes"] TO 1
        )
...
def run_eval(model, loader):
    ...
    total_loss = 0
    criterion = nn.BCEWithLogitsLoss() # Match the training loss
    ...
    with torch.no_grad():
        for imgs, labels in loader:
            ...
            # Flatten back to 1D lists for sklearn metrics
            all_preds.extend(preds.cpu().numpy().flatten())
            all_labels.extend(labels.cpu().numpy().flatten())
            
    acc = (torch.tensor(all_preds) == torch.tensor(all_labels)).float().mean().item() * 100
    
    # Change average to 'binary'
    f1 = f1_score(all_labels, all_preds, average='binary') 
    return acc, total_loss / len(loader), f1, all_preds, all_labels

def train():
    wandb.init(project="cross-out-detection", name="Yolo11 - Binary - FeatureExtraction - No Preprocessing - ReduceLROnPlateau", config=CONFIG) # Setup wandb
    ...
    actual_model = model.module if isinstance(model, nn.DataParallel) else model #Unwrap the model from DataParallel
    ...
    patience_counter = 0
    best_model_path = "Best_models/Yolo11_FeatureExtractor_Binary_NoPreprocessing_ReduceLROnPlateau.pth"
    ...
    for epoch in range(CONFIG["epochs"]):
        ...
        for imgs, labels in tqdm(train_loader, desc=f"Epoch {epoch+1}"):
            ...
            # FORMAT LABELS FOR BINARY LOSS: shape becomes [128, 1]
            labels = labels.float().unsqueeze(1) 
            ...
            # PREDICTIONS FOR BINARY: If logit > 0, prediction is 1, else 0
            preds = (out > 0.0).float()
            correct += (preds == labels).sum().item()
            total += labels.size(0)
            
        # ---> THIS IS OUTSIDE THE BATCH LOOP <---
