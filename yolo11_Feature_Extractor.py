...
def train():
    wandb.init(project="cross-out-detection", name="Yolo11 - Multiclass - FeatureExtraction - No Preprocessing", config=CONFIG) # Setup wandb
    
    transform = transforms.Compose([
        #transforms.Resize((CONFIG["img_size"], CONFIG["img_size"])), # No longer needed Resizing is done in CrossOutDataset(Dataset)
        transforms.ToTensor(),
        transforms.Lambda(lambda x: 1.0 - x),
        transforms.Normalize(mean=[0.0608, 0.0608, 0.0608], 
                             std=[0.1927, 0.1927, 0.1927]) # Calculated with mean_std script
    ])
    ...
    optimizer = optim.Adam(actual_model.head.parameters(), lr=CONFIG["lr"])
    criterion = nn.CrossEntropyLoss()
    ...
    patience_counter = 0
    best_model_path = "Best_models/Yolo11_FeatureExtractor_MultiClass_NoPreprocessing.pth"
